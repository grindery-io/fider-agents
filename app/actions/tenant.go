...
// UpdateTenantFeedSettings is the input model used to update tenant advanced settings
type UpdateTenantFeedSettings struct {
	IsFeedEnabled bool   `json:"isFeedEnabled"`
	FeedAppId     string `json:"feedAppId"`
	FeedApiKey    string `json:"feedApiKey"`
	FeedApiSecret string `json:"feedApiSecret"`
}

// IsAuthorized returns true if current user is authorized to perform this action
func (action *UpdateTenantFeedSettings) IsAuthorized(ctx context.Context, user *entity.User) bool {
	return user != nil && user.Role == enum.RoleAdministrator
}

// Validate if current model is valid
func (action *UpdateTenantFeedSettings) Validate(ctx context.Context, user *entity.User) *validate.Result {
	return validate.Success()
}
